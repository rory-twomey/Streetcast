  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 relative">
      ...
      <div className="flex items-center justify-center gap-5 px-5 pt-4 pb-2">
        <ActionButton variant="pass" icon={<X size={22} strokeWidth={2.4} />} />
        <ActionButton variant="info" icon={<Info size={17} strokeWidth={2.4} />} />
        <ActionButton variant="accept" icon={<Check size={26} strokeWidth={2.4} />} />
      </div>
...
function ActionButton({
  ...
  if (variant === "accept") {
    return (
      <button className={`${base} w-16 h-16`} style={{ background: "var(--green)", color: "#fff" }}>
        {icon}
      </button>
    );
  }
  ...
  return (
    <button className={`${base} w-13 h-13`} style={{ background: "var(--red)", color: "#fff" }}>
      {icon}
    </button>
  );
}
